...
def feature_engineering(data_ml_drop):
    ...
    missing_cols = [col for col in REQUIRED_COLUMNS if col not in data_ml_drop.columns]
    if missing_cols:
    ...
    data_ml_drop = data_ml_drop.loc[:, ~data_ml_drop.columns.isin(['level_0', 'index'])]

    # Feature engineering
    data_ml_drop['loan_burden'] = ((data_ml_drop['housing'] == 'yes') & 
                                   (data_ml_drop['loan'] == 'yes')).astype(int)
